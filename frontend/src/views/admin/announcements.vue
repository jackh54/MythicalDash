<template>
    <LayoutDashboard>
        ...
        <TableTanstack :data="Announcements" :columns="columnsAnnouncements" tableName="Announcements" />
    </LayoutDashboard>
</template>
...
const columnsAnnouncements = [
    ...
    },
    {
        accessorKey: 'tags',
        header: 'Tags',
        cell: ({ row }: { row: { original: { tags: string[] } } }) => row.original.tags.join(', '),
    },
    {
        accessorKey: 'actions',
        header: 'Actions',
        cell: ({ row }: { row: { original: { tags: string[]; id: number } } }) =>
            h(
                'button',
                {
